...
src/
  ...
  nexusgpu/
    ...
    useCameraControls.ts Canvas上のドラッグ、ホイール、ピンチ、WASD、Q/Eをカメラ更新へ変換するhook
...
- `useCameraControls()`へ`canvasRef`、`camera`、`orbitControls`、`wasdControls`、`SceneStore`を渡す
...
Canvas上のユーザー入力をcamera更新へ変換するhookです。`NexusCanvas`本体からカメラ操作のDOMイベント処理を分離し、`NexusCanvas`は描画基盤の配線に集中します。
...
`useFrame`のcallbackには`time`、`elapsed`、`delta`が渡されます。SDFオブジェクトを動かす場合は、callback内でReact stateを更新し、そのstateを`<SdfSphere position={...} />`や`<SdfBox position={...} />`へ渡します。`useCamera()`で更新したcameraは`useCameraControls()`側にも同期されるため、スクリプト制御を止めた後に`orbitControls`や`wasdControls`でその位置から操作できます。スクリプト制御とユーザー操作を同時に走らせる場合は、最後に`SceneStore.setCamera()`した側の更新が表示に反映されます。
