...
`renderSettings`、選択中scene、シーン固有パラメータのstateを保持し、選択中scene componentと各パネルへ渡します。`App.tsx`自体には個別sceneのimportやプリミティブ定義、scene camera / lightingを置かず、`src/scenes/registry.ts`から選択中のscene定義を受け取ります。
...
`src/scenes/scenes.json`は、アプリで切り替え可能なsceneをまとめる薄い登録ファイルです。`src/scenes/registry.ts`はJSONを読み、`module`に対応するtsxファイルを`import.meta.glob`で解決し、sceneファイルがexportする`Scene`、任意の`initialParameters`、`parameterControls`と結合して`NexusSceneDefinition`へ変換します。`initialParameters`がないsceneは空 object として扱います。`App.tsx`は個別sceneを直接importせず、registryの定義だけを参照します。
...
これらはsceneやアプリの推奨初期値ではなく、ライブラリ単体で安全に動くためのfallbackです。sceneごとに見え方を変える場合は、sceneファイルの`Scene`が返す`NexusCanvas`へcamera / lightingを明示的に渡します。
...
sequenceDiagram
  ...
  Scene->>NexusCanvas: <NexusCanvas camera lighting>をレンダー
