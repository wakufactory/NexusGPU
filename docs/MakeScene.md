...
| `SdfGroup` | 子SDFのboolean合成 | `active`, `op`, `position`, `rotation`, `smoothness`, `material`, `materialUniform`, `bounds`, `children` |
| `SdfNot` | 子SDFの内外反転 | `active`, `children` |
| `SdfSubtract` | 1つ目の子から後続の子を差し引く | `active`, `children` |
| `SdfMix` | 2つの子SDFをratioで線形補間 | `active`, `ratio`, `children` |
| `SdfModifier` | 子SDFの評価前後にWGSL modifierを差し込む | `active`, `preset`, `preModifierFunction`, `postModifierFunction`, `data0`, `data1`, `data2`, `children` |

共通props:

- `position`: `[x, y, z]`。省略時は`[0, 0, 0]`
- `active`: `false`を指定すると、そのprimitive / group / mixはscene graphへ登録されず、rendererやshader生成から完全に除外される。`SdfModifier`ではmodifier機能だけを停止し、子SDFを通常の`op="or"` groupとして扱う。省略時は`true`
...
- `SdfFunction.bounds`: 明示的な枝刈り用bounding sphere。`center`はSdfFunctionローカル座標で、省略時は`[0, 0, 0]`
...
- `SdfGroup.bounds`: 明示的な枝刈り用bounding sphere。`center`はgroupローカル座標で、省略時は`[0, 0, 0]`
- `SdfMix.ratio`: 2つの子SDFを補間する比率。`0`なら1つ目、`1`なら2つ目へ寄る。省略時は`0.5`
- `SdfModifier.preset`: `"twistY"`、`"preRepeat"`、`"preScale"`、`"postInflate"`、`"postOnion"`、またはそれらの配列
...
枝刈りしたいcustom SDFまたはまとまりには、`SdfFunction.bounds`または`SdfGroup.bounds`で保守的なbounding sphereを明示します。自動bounds計算は行わず、`bounds`を持つfunction / groupだけが`mapSceneDistance()`でGPU枝刈りされます。
...
組み込みpresetは次の5つです。presetはpre/postの片方だけでなく、両方を持つ場合があります。
...
`SdfModifier`自体はboundsを持ちません。modifierを含むまとまりを枝刈りしたい場合は、外側の`SdfGroup`へ保守的な`bounds`を指定してください。任意pre modifierは評価空間を曲げるため、実際の変形後形状が必ず収まる半径を指定する必要があります。

`SdfModifier.data0`、`data1`、`data2`と`SdfMix.ratio`はStorage Bufferへ入るため、値だけを変えた場合はShader Module / Render Pipelineを作り直しません。`preset`、`preModifierFunction`、`postModifierFunction`、children構造が変わった場合は、生成される`mapSceneDistance()` / `mapSceneEval()`やcustom WGSL関数が変わるためpipeline再生成が発生します。
...
| preset | 内容 |
| --- | --- |
| `"default"` | 標準のambient / diffuse / shadow / rim material。`material`未指定時もこれを使う |
...
`"default"`では`materialUniform`を`[ambient, rim, 0, 0]`として使います。未指定または`ambient`が0の場合、ambientは従来値の`0.54`として扱います。`rim`は0なら無効、値を上げると輪郭の発光寄りのハイライトが強くなります。
...
<NexusCanvas
  ...
  textures={[
    {
      src: new URL("../assets/tex1024.png", import.meta.url).href,
      addressModeU: "repeat",
      addressModeV: "repeat",
      magFilter: "linear",
      minFilter: "linear",
...
scene専用の画像は、sceneファイルの近くにある`assets`フォルダへ置き、`new URL("../assets/tex1024.png", import.meta.url).href`で参照します。Viteがdev/buildの両方で正しいURLへ解決し、build時は`dist/assets`へハッシュ付きで出力します。外部URLを使う場合は、必要に応じて`crossOrigin: "anonymous"`を指定します。ただし配信元がCORSを許可していない画像はブラウザ側で読み込めません。
