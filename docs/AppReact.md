...
`camera`、`lighting`、scene componentは`registry.ts`から受け取ります。registryは`scenes.json`の`module`を`import.meta.glob`で解決します。
...
sceneファイルは`Scene` componentをexportし、sceneに必要な初期値は`scenes.json`へ書きます。
...
次に`src/scenes/scenes.json`へscene定義を追加します。`registry.ts`が`module`を解決するため、`App.tsx`のimportやJSXをsceneごとに差し替える必要はありません。
...
2. sceneに合う`camera`と`lighting`を`scenes.json`へ用意する
3. 初期値を`scenes.json`の`initialParameters`へ用意する
