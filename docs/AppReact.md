...
`camera`、`lighting`、scene componentは`registry.ts`から受け取ります。registryは`scenes.json`の`module`を`import.meta.glob`で解決し、sceneファイルの個別exportから`camera`と`lighting`を取得します。
...
sceneファイルは`Scene` componentと、`camera`、`lighting`、`initialParameters`、必要なら`parameterControls`をexportします。sceneに必要な初期値は同じTSXへ書き、`scenes.json`は一覧表示用の`id`、`title`、`description`、`module`だけを持ちます。
...
- 同じtsxファイルの`camera`、`lighting`、`initialParameters`
- 薄い`scenes.json`登録
...
同じsceneファイルに`camera`、`lighting`、`initialParameters`を書き、次に`src/scenes/scenes.json`へ薄いscene定義を追加します。`registry.ts`が`module`を解決するため、`App.tsx`のimportやJSXをsceneごとに差し替える必要はありません。
...
2. sceneに合う`camera`と`lighting`を用意する
