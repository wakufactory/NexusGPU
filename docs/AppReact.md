...
重要なのは、sceneもReactコンポーネントだという点です。sceneの`Scene` exportは`NexusCanvas`まで含む通常のReact componentで、その内側に`<SdfBox />`や`<SdfSphere />`を置きます。
...
- scene componentへscene固有パラメータと`canvasProps`を渡す
...
return (
  <main>
    <section className="viewport">
      <Scene parameters={sceneParameters} canvasProps={canvasProps} />
    </section>
...
実際のコードでは、ここにフルスクリーンボタン、state、更新関数が加わります。カメラとライトはsceneファイル内の`NexusCanvas`へ書きます。
...
`renderSettings`はレンダリング品質や表示方式のような、`NexusCanvas`全体に関わる値です。`App`はこれを`canvasProps`としてsceneへ渡し、scene側の`NexusCanvas`が受け取ります。
...
`NexusCanvas`は、Reactツリーの中でsceneを表示するための領域です。デモアプリでは各sceneファイルの`Scene` componentが`NexusCanvas`を作ります。
...
React視点では、`NexusCanvas`はchildrenを受け取るcomponentです。childrenに置いたscene contentは、`NexusCanvas`の内側で実行されます。

scene componentは`registry.ts`から受け取ります。registryは`scenes.json`の`module`を`import.meta.glob`で解決し、sceneファイルの`Scene`、任意の`initialParameters`、`parameterControls`を取得します。`initialParameters`がないsceneは空 object として扱われます。
...
sceneごとの見え方はsceneファイル内の`NexusCanvas` propsで定義します。`App`は`canvasProps`として共通のレンダリング制御だけを渡します。
...
export function Scene({ parameters, canvasProps }: AnimatedSdfSceneProps) {
  return (
    <NexusCanvas {...canvasProps} camera={...} lighting={...} orbitControls>
...
`NexusCanvas`の内側のscene contentはDOM要素を返す代わりに、`SdfBox`や`SdfSphere`のようなNexusGPUのprimitive componentを返します。
...
sceneファイルは`Scene` component、必要なら`initialParameters`と`parameterControls`をexportします。カメラとライトは`Scene`が返す`NexusCanvas` propsに書きます。sceneに必要な初期値は同じTSXへ書き、`scenes.json`は一覧表示用の`id`、`title`、`description`、`module`だけを持ちます。
...
- 同じtsxファイルの`NexusCanvas`
- scene固有パラメータがある場合は`initialParameters`
- 薄い`scenes.json`登録
...
カメラやライト自体を動かす場合は、`NexusCanvas`の内側のcomponentで`useCamera()`や`useLighting()`を使います。
...
同じsceneファイルにscene contentを書き、scene固有パラメータがある場合は`initialParameters`も用意します。次に`src/scenes/scenes.json`へ薄いscene定義を追加します。`registry.ts`が`module`を解決するため、`App.tsx`のimportやJSXをsceneごとに差し替える必要はありません。
...
2. `Scene`が返す`NexusCanvas`にsceneに合う`camera`と`lighting`を書く
3. scene固有パラメータがある場合は、初期値を`initialParameters`へ用意し、`typeof initialParameters`で型を作る
