...
sceneファイルは`Scene` component、必要なら`defineSceneParameterControls`で作った`initialParameters`と`parameterControls`をexportします。カメラとライトは`Scene`が返す`NexusCanvas` propsに書きます。sceneに必要な初期値は同じTSXへ書き、`scenes.json`は一覧表示用の`id`、`title`、`description`、`module`だけを持ちます。
...
- scene固有パラメータがある場合は`defineSceneParameterControls`で作る`initialParameters`と`parameterControls`
- 薄い`scenes.json`登録
...
3. scene固有パラメータがある場合は、`defineSceneParameterControls`で初期値とslider定義をまとめ、`typeof initialParameters`で型を作る
